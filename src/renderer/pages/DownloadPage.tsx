...
import { ChevronDown, FolderOpen, Pause, Play, Plus, Search, X } from 'lucide-react';
...
export const DownloadPage: React.FC<DownloadPageProps> = ({
  ...
  const liveTasks = [...downloads.active, ...downloads.waiting, ...downloads.stopped];
  const currentTasks = (() => {
    if (currentCategory === 'downloading') {
      return liveTasks.filter((task: any) => ['active', 'waiting'].includes(task.status));
    }
    ...
    if (currentCategory === 'deleted') {
      return historyTasks.filter((task: any) => task.status === 'error' || task.status === 'removed');
    }
  ...
  const pausableTasks = currentTasks.filter((task: any) => task.status === 'active');
  const resumableTasks = currentTasks.filter((task: any) => task.status === 'paused');
  ...
  const selectedHistoryTask = selectedTaskGid
    ...
    : null;
  const showDetailDrawer = Boolean(selectedTaskGid && (selectedTask || selectedHistoryTask));
  ...
  return (
    <>
      <div className="relative grid min-h-0 flex-1 grid-cols-1 grid-rows-[38px_minmax(0,1fr)] gap-y-4 overflow-hidden">
        <div className="flex h-[38px] items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <div className="relative w-[160px]">
              <Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#9A8276]" />
              <input
                value={searchTerm}
                onChange={(event) => setSearchTerm(event.target.value)}
                placeholder={t('searchPlaceholder')}
                className="h-[34px] w-full rounded-[16px] border border-[#F0DED8] bg-white/90 pl-9 pr-3 text-[12px] text-[#6B5448] shadow-sm outline-none transition-all placeholder:text-[#B7A59C] focus:border-[#FFC3CF] focus:ring-4 focus:ring-[#FFE6EC]"
              />
            </div>
            <div className="flex h-[34px] w-[34px] items-center justify-center rounded-full border border-[#F0DED8] bg-white/90 shadow-sm">
              <img src={faviconUrl} alt="Grabbit" className="h-[26px] w-[26px] object-contain" />
            </div>
          </div>
        </div>
        ...
        {showDetailDrawer && (
          <div className="absolute bottom-0 right-0 top-0 z-30 w-[min(520px,72%)] min-w-[430px]">
            <DownloadDetail
              task={selectedTask}
