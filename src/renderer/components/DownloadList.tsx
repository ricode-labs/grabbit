...
export const DownloadList: React.FC<DownloadListProps> = ({
  ...
  const filteredTasks = filterTasksByCategory().filter(task => {
    ...
    const fileName = task.files?.[0]?.path?.split('/').pop() || task.fileName || '';
    const dir = task.dir || '';
  ...
  if (filteredTasks.length === 0) {
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center rounded-[17px] border border-[#F6D7D3] bg-white/70">
        <img src={emptyUrl} alt="" className="mb-3 h-[122px] w-[122px] object-contain opacity-95" />
        <p className="text-[15px] font-medium text-[#6B5448]">{emptyMessages[category]}</p>
    ...
    <div className="h-full overflow-hidden rounded-[17px] border border-[#F6D7D3] bg-white/82 shadow-[0_10px_28px_rgba(107,84,72,0.06)]">
      <div className="h-full overflow-y-auto">
        {filteredTasks.map(task => {
          ...
          const liveTask = task.gid ? allTasks.find(t => t.gid === task.gid) : null;
