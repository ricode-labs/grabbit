...
export const DownloadDetail: React.FC<DownloadDetailProps> = ({
  ...
  const { t } = useUI();
  const fileName = task?.files?.[0]?.path?.split('/').pop() || historyTask?.fileName || t('unknown');
  ...
  const completedLength = Number(task?.completedLength || historyTask?.completedLength || 0);
  const totalLength = Number(task?.totalLength || historyTask?.totalLength || 0);
  const speed = Number(task?.downloadSpeed || 0);
  const progress = totalLength > 0 ? Math.min((completedLength / totalLength) * 100, 100) : 0;
  ...
  const infoRows = [
    ...
    { label: t('fileSize'), value: `${formatBytes(totalLength)}${totalLength ? ` (${totalLength.toLocaleString()} ${t('bytes')})` : ''}` },
    { label: t('downloaded'), value: `${formatBytes(completedLength)}${completedLength ? ` (${completedLength.toLocaleString()} ${t('bytes')})` : ''}` },
    ...
    { label: t('supportsResume'), value: isComplete || isActive || isPaused ? t('yes') : '-' },
    { label: t('server'), value: getServer() },
    { label: t('etag'), value: task?.verifiedLength ? `"${task.verifiedLength}"` : '-' }
  ];
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col overflow-hidden rounded-[18px] border border-[#F2DED6] bg-[#FFFBF8]/98 shadow-[0_18px_46px_rgba(107,84,72,0.18)] backdrop-blur-sm">
      <header className="flex h-[58px] shrink-0 items-center justify-between border-b border-[#F4E3DE] px-5">
        <h2 className="text-[18px] font-semibold text-[#2D2522]">{t('downloadDetail')}</h2>
        <div className="flex items-center gap-3 text-[#8B6A5D]">
          <button className="flex h-7 w-7 items-center justify-center rounded-full hover:bg-[#FFF1F4]" title={t('pin')}>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <section className="shrink-0 px-5 pb-3 pt-5">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="mb-2 flex items-start gap-2">
                <h3 className="min-w-0 flex-1 break-words text-[16px] font-semibold leading-snug text-[#2D2522]">{fileName}</h3>
                <span className={`shrink-0 rounded-full px-2 py-1 text-[11px] font-semibold leading-none ${getStatusTone()}`}>
                  {getStatusLabel()}
                </span>
              </div>
        ...
        <nav className="grid h-11 shrink-0 grid-cols-4 border-b border-[#F4E3DE] px-5 text-[13px] font-medium text-[#6B5448]">
          <button className="relative flex items-center justify-center gap-1.5 text-[#FF5C78]">
            <Info size={16} />
            {t('detailInfo')}
            <span className="absolute bottom-0 left-2 right-2 h-[2px] rounded-full bg-[#FF7D90]" />
          </button>
        ...
        <main className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
          <div className="overflow-hidden rounded-[12px] border border-[#F4E3DE] bg-white/58">
            {infoRows.map((row, index) => (
              ...
              >
                <span className="pt-0.5 text-[13px] text-[#6B5448]">{row.label}</span>
                <span className={`min-w-0 whitespace-normal break-all text-right text-[13px] leading-relaxed ${row.link ? 'text-[#2E8BD6]' : 'text-[#7A6257]'}`}>
                  {row.value}
                </span>
